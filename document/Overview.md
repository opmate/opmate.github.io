...
실행 인스턴스는 인스턴스 번호를 가지고 있는데, 전체 태스크에서 고유한 번호이며 태스크 실행마다 새로운 번호가 부여된다.

태스크ID를 지정하여 실행 인스턴스를 확인 할 경우 인스턴스 목록을 확인할 수 있다.

#### 노드 인스턴스 (Task Node Instance)

실행 인스턴스를 지정하여 상세 조회시, 노드별 **진행 상태**와 **실행 결과**를 확인할 수 있다.
