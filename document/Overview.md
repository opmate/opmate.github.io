...
노드를 비활성화하면 해당 노드가 특정 태스크의 대상 노드 (Target Node)로 등록되어 있더라도, 태스크 실행시 제외된다.
...
현 태스크의 실행이 정상적으로 완료(모든 타겟노드의 실행결과가 성공)된 후 자동으로 다른 태스크가 실행되게 할 경우에 지정한다.

단, 호출되는 태스크에서 실행 권한을 부여해줘야 한다.
예를 들어, A태스크와 B태스크가 C태스크를 Next Task로 실행하고자 한다면,  A와 B 태스크가 실행 가능하다는 것을 C태스크에서 권한설정을 해줘야 한다.
...
각각의 태스크 인스턴스는 전체 태스크에서 유일한 **태스크 인스턴스 번호 (Task Instance No)** 가 부여된다.

태스크ID를 지정하여 특정 태스크로부터 생성(실행)된 태스크 인스턴스 목록을 확인할 수 있다.

#### 태스크 인스턴스 노드별 상세 (Task Node Instance)

태스크 인스턴스 번호와 노드를 지정하면 태스크 인스턴스의 노드별 상세를 확인할 수 있다.
...
- **실행 결과 코드**
  - 쉘스크립트의 최종 종료값
- **표준출력/표준에러**
  - 쉘스크립트의 실행결과인 표준출력과 표준에러

#### 태스크 인스턴스 노드 단위 재실행

이미 생성된 태스크 인스턴스는 특정 노드를 지정하여 노드 단위로 재실행할 수 있다.

단, 실행 인스턴스의 생성 시점 이후에 해당 태스크의 리비젼이 변경되었을 경우 재실행은 불가하다.

## 파일 배포/수집 유틸리티

노드별로 설치되는 에이전트에 포함된 유틸리티이며, 쉘스크립트에 기술하여 실행할 수 있다.
